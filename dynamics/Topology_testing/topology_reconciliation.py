...
def calculate_expected_topology_changes(events_dict: Dict) -> Dict[str, Tuple[int, int]]:
    ...
    3. Tip-tip fusion: Δtips = -2, Δjunctions = 0 (conservative, could be +1)
    4. Tip-tip fission: Δtips = +2, Δjunctions = 0 (conservative, could be -1)
    ...
    expected_delta_junctions = (
        1 * n_tip_edge_fusion +
        -1 * n_junction_breakage +
        0 * n_tip_tip_fusion +  # Conservative estimate
        0 * n_tip_tip_fission +  # Conservative estimate
        1 * n_extrusion +
        -1 * n_retraction
    )
