...
Time series adjacency list CSV reader for analyzing network dynamics.
"""
...
def read_timeseries_csvs(base_folder: str) -> pd.DataFrame:
    """
    Read all adjacency list CSVs from time series folders and combine into single DataFrame.

    Args:
        base_folder: Path to the base folder containing time point subdirectories (1, 2, 3, etc.)

    Returns:
        Combined pandas DataFrame with all adjacency data and time_point column
    """
    ...
    for time_point in time_points:
        ...
        # Look for adjacency list CSV files
        adjacency_files = glob.glob(os.path.join(nellie_path, '*_adjacency_list.csv'))

        if not adjacency_files:
            print(f"  No adjacency list CSV found")
            continue
        elif len(adjacency_files) > 1:
            print(f"  Multiple adjacency CSVs found, using first: {adjacency_files[0]}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        df.to_csv(os.path.join(base_folder, "combined_timeseries_adjacency.csv"), index=False)
        print(f"\nFirst few rows:")
        print(df.head())
        print(f"\nDatabase info:")
        print(df.info())
    else:
        print("Usage: python timeseries_reader.py <base_folder_path>")